...
- Paired 60-second, copyright-free judge fixtures now present a plausible narrated video-essay package: the defective cut has one black export gap, a delayed promise, one visible unfinished placeholder, and one incorrect historical date; the corrected cut removes them.
...
- The creator-style M15 package uses macOS `say` for reproducible local narration. Its technical, Promise, and Viewer evidence was live-verified, but Claim Review abstained on the intended Apollo date claim within the allowed acceptance-call budget.
...
- Milestone 15 generator — produced ignored 1280×720, 15 fps, 60-second defective (approximately 8.7 MB) and corrected (approximately 8.5 MB) MPEG-4/AAC videos plus a 6.8 KB aligned PNG thumbnail from local graphics and macOS speech synthesis.
- Milestone 15 deterministic package validation — defective `NEEDS_REVIEW`, 20 checks, 19 passed, 1 warning, 0 critical, with only the intended black interval at 12.0–15.066667 seconds and 100% caption coverage; corrected `READY`, 20/20 passed, 0 warnings, 0 critical, also with 100% caption coverage.
- Milestone 15 first live defective scan — `NEEDS_REVIEW`, 23 checks, 20 passed, 4 warnings, 0 critical. Findings were Promise delay 0–24 seconds, black 12.0–15.066667 seconds, narration/visual conflict 35.5–47 seconds, and visible placeholder 48–60 seconds. Claim Review returned `no_claims`; one upload, three video generations, and one cleanup completed in approximately 26.55 seconds.
- Milestone 15 pre-clarification live corrected scan — `READY`, 23/23 passed, 0 warnings, 0 critical. Promise was aligned from 0 seconds, Viewer Pass was clean, Claim Review returned `no_claims`, and one upload/three video generations/one cleanup completed in approximately 23.83 seconds. The current corrected generator has the narrowly clarified 1969 narration but was not re-sent to Gemini after the live-call budget was exhausted; its current deterministic/browser control is verified `READY`.
- Milestone 15 allowed corrective defective retry — `NEEDS_REVIEW`, 23 checks, 20 passed, 3 warnings, 0 critical. Promise delay remained 0–24 seconds; black remained 12.0–15.066667 seconds; the visible `TODO REPLACE THIS MAP` placeholder was found at 47–59 seconds. Claim Review again selected zero claims, so no grounding request or claim finding occurred. One upload, three generations, and one cleanup completed in approximately 24.443 seconds (upload 5.411s, Promise 4.411s, Viewer 4.094s, extraction 4.812s, cleanup 0.319s).
- Milestone 15 browser audit at 1440, 1024, and 430 CSS pixels — no document overflow or console warnings/errors; the defective package rendered the real deterministic result and both finding-timecode and timeline-marker controls sought to exactly 12 seconds. New scan cleared the package, and the current corrected package rendered `READY` with 20/20 checks.
- Milestone 15 release gate — `.venv/bin/python -m pytest backend/tests`: 167 passed, 0 failed, 1 upstream Starlette warning; `cd frontend && npm test -- --run`: 31 passed, 0 failed; `npm run build`: TypeScript and Vite production build passed with 1,825 modules transformed; Python compile, shell syntax, and `git diff --check` passed.
- Final `./scripts/run_demo.sh` regression — passed and restored the established deterministic `NEEDS_REVIEW` result with 15 passed, 5 warnings, 0 critical; black 2–5s, silence 3–6s, freeze 7–10s, calibrated audio warning, and title recommendation.
- Anonymous access to `https://github.com/Talanezi/creator-preflight` returned HTTP 200, and the configured remote was reachable; repository visibility is public and is not a submission blocker.
- M15 fixture-only continuation, first live defective scan — `NEEDS_REVIEW`, 23 checks, 21 passed, 2 warnings, 0 critical. Black remained 12.0–15.066667s and the placeholder remained 48–60s. Promise returned `not_evaluable` at 0.65 confidence with substantive address at 24s; Viewer raw output interpreted spoken 1969 versus visible 1968 but suppressed the 0.72-confidence conflict and retained the 0.75 placeholder. Claim extraction returned zero claims. One upload, three video generations, no grounding call, one successful cleanup; AI runtime approximately 24.61s.
- M15 fixture-only corrective retry — changed only the Apollo slide to `APOLLO 11 MOON LANDING / YEAR 1968` and narration to “Apollo eleven landed on the Moon in the year one nine six eight”; corrected media uses 1969 identically. The live defective report was `NEEDS_REVIEW`, 23 checks, 21 passed, 2 warnings, 0 critical: Promise delay 0–24s and black 12.0–15.066667s. Promise was aligned at 0.72 confidence; Viewer was conservatively clean because its recognized placeholder issue contained no evidence list; Claim extraction again returned zero claims. One upload, three video generations, no grounding call, one successful cleanup; runtime approximately 23.83s.
- M15 continuation targeted validation — `PYTHONPATH=backend/src .venv/bin/python -m pytest backend/tests/test_claim_review.py backend/tests/test_promise_check.py backend/tests/test_viewer_pass.py`: 34 passed, 0 failed. `git diff --check` passed. No production logic, AI thresholds, provider code, frontend code, or secrets changed in the continuation.
